...
int main()
{
    ...
    for (int x = 0; x < len; x++) {
        result[x] = (int*)malloc(sizeof(int) * len);
    }
    ...
    for (int t = 2; t < len; t++) {
        for (int i = 1; i < len - t + 1; i++) {
            jj = i + t - 1;
            result[i][jj] = 100000;
            for (int k = i; k <= jj - 1; k++) {
                qq = (int)result[i][k] + (int)result[k + 1][jj] + (int)list[i - 1] * (int)list[k] * (int)list[jj];
                if (qq < result[i][jj])
                    result[i][jj] = qq;
            }
        }
    }
    int outputt = result[1][len - 1];
    printf("/////////////////\n");
    ...
    printf("Minimizes The Total Number Of Multiplications:");
    printf("%d\n", outputt);
    printf("\n");
    // ------------------------------------------------
    ...
    fprintf(file2, "Minimizes The Total Number Of Multiplications : %d", outputt);
    fprintf(file2, "\nOutput Matrix\n");
    for (int j = 0; j < (int)save[0]; j++) {
        for (int k = 0; k < (int)save[(int)((line * 2) - 1)]; k++) {
